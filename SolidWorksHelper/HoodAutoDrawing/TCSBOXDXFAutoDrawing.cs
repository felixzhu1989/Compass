...
{
    public class TCSBOXDXFAutoDrawing : IAutoDrawing
    {
        readonly TCSBOXDXFService objTCSBOXDXFService = new TCSBOXDXFService();
        ...
        public void AutoDrawing(SldWorks swApp, ModuleTree tree, string projectPath)
        {
            ...
            //查询参数
            TCSBOXDXF objTCSBOXDXF = (TCSBOXDXF)objTCSBOXDXFService.GetModelByModuleTreeId(tree.ModuleTreeId.ToString());
            ...
            if (objProject.HoodType == "HoodParent")
            {
                ...
                foreach (var item in oldList)
                {
                    ...
                    {
                        ModuleTreeId = objTCSBOXDXF.ModuleTreeId,
                        PartDescription = item.PartDescription,
                        Length = item.Length,
                        Width = item.Width,
                        Thickness = item.Thickness,
                        Quantity = item.Quantity * objTCSBOXDXF.Quantity,//多个UCP
                ...
                {
                    throw new Exception("TCSBOXDXF的HoodCutlist导入数据库失败" + ex.Message);
                }
            }
            #endregion
            ...
            if (objProject.HoodType == "Ceiling")
            {
                ...
                {
                    throw new Exception("TCSBOXDXF的CeilingCutlist导入数据库失败" + ex.Message);
                }
            }
            #endregion
        }
