...
{
    public class KVI400AutoDrawing : IAutoDrawing
    {
        readonly KVI400Service objKVI400Service = new KVI400Service();
        public void AutoDrawing(SldWorks swApp, ModuleTree tree, string projectPath)
        {
            ...
            //查询参数
            KVI400 item = (KVI400)objKVI400Service.GetModelByModuleTreeId(tree.ModuleTreeId.ToString());
            ...
            */
            ...
            try
            {
                //----------Top Level----------
                //烟罩深度
                swModel.Parameter("D1@Distance40").SystemValue = item.Deepth / 1000d;
                ...
                //KSA距离左边缘
                if (ksaSideLength < 12d / 1000d) swModel.Parameter("D1@Distance4").SystemValue = 0.5d / 1000d;
                ...
                if (item.ExNo == 1)
                {
                    swModel.Parameter("D1@Distance2").SystemValue = (item.ExRightDis - item.ExLength / 2) / 1000d;
                ...
                //----------排风腔----------
                swComp = swAssy.GetComponentByName(CommonFunc.AddSuffix(suffix, "FNHE0143-1"));
                ...
                {
                    swFeat.SetSuppression2(1, 2, configNames); //参数1：1解压，0压缩
                    if (item.ExNo == 1) swPart.Parameter("D1@Sketch22").SystemValue = (item.ExRightDis + item.ExLength / 2 + 50d) / 1000d;
                    else swPart.Parameter("D1@Sketch22").SystemValue = (item.ExRightDis + item.ExDis / 2 + item.ExLength + 50d) / 1000d;
                }
                ...
                //if (item.ANSUL == "YES")
                //{
                //    swComp = swAssy.GetComponentByName(CommonFunc.AddSuffix(suffix, "FNHE0145-1"));
                ...
                if (item.SidePanel == "BOTH")
                {
                    ...
                    swComp.SetSuppression2(2); //2解压缩，0压缩
                    swEdit.FNHS0001(swComp, item.Deepth, 400d, sidePanelSideCjNo, sidePanelDownCjNo);

                    swComp = swAssy.GetComponentByName(CommonFunc.AddSuffix(suffix, "FNHS0002-1"));
                    swComp.SetSuppression2(2); //2解压缩，0压缩
                    swEdit.FNHS0002(swComp, item.Deepth, 400d);

                    //RIGHT
                    swComp = swAssy.GetComponentByName(CommonFunc.AddSuffix(suffix, "FNHS0003-1"));
                    swComp.SetSuppression2(2); //2解压缩，0压缩
                    swEdit.FNHS0003(swComp, item.Deepth, 400d, sidePanelSideCjNo, sidePanelDownCjNo);

                    swComp = swAssy.GetComponentByName(CommonFunc.AddSuffix(suffix, "FNHS0004-1"));
                    swComp.SetSuppression2(2); //2解压缩，0压缩
                    swEdit.FNHS0004(swComp, item.Deepth, 400d);
                    ...
                    if (item.WaterCollection == "YES")
                    {
                        ...
                        swComp.SetSuppression2(2); //2解压缩，0压缩
                        swPart = swComp.GetModelDoc2();
                        swPart.Parameter("D2@Base-Flange1").SystemValue = (item.Deepth - 275d) / 1000d;//水洗烟罩(item.Deepth - 368) / 1000d;
                        swPart.Parameter("D5@Sketch7").SystemValue = 13.68d / 1000d;//水洗烟罩19.87d / 1000d
                ...
                //------------I型新风腔主体----------
                swComp = swAssy.GetComponentByName(CommonFunc.AddSuffix(suffix, "FNHA0040-1"));
                swPart = swComp.GetModelDoc2();
