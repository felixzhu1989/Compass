...
{
    public class MCPDXFAutoDrawing : IAutoDrawing
    {
        readonly MCPDXFService objMCPDXFService = new MCPDXFService();
        ...
        public void AutoDrawing(SldWorks swApp, ModuleTree tree, string projectPath)
        {
            ...
            //查询参数
            MCPDXF objMCPDXF = (MCPDXF)objMCPDXFService.GetModelByModuleTreeId(tree.ModuleTreeId.ToString());
            ...
            if (objProject.HoodType == "HoodParent")
            {
                ...
                foreach (var item in oldList)
                {
                    ...
                    {
                        ModuleTreeId = objMCPDXF.ModuleTreeId,
                        PartDescription = item.PartDescription,
                        Length = item.Length,
                        Width = item.Width,
                        Thickness = item.Thickness,
                        Quantity = item.Quantity * objMCPDXF.Quantity,//多个UCP
                ...
                {
                    throw new Exception("MCPDXF的HoodCutlist导入数据库失败" + ex.Message);
                }
            }
            #endregion
            ...
            if (objProject.HoodType == "Ceiling")
            {
                ...
                {
                    throw new Exception("MCPDXF的CeilingCutlist导入数据库失败" + ex.Message);
                }
            }
            #endregion
        }
