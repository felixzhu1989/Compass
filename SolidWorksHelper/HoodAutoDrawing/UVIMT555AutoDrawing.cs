    ...
    public class UVIMT555AutoDrawing : IAutoDrawing
    {
        ...
        public void AutoDrawing(SldWorks swApp, ModuleTree tree, string projectPath)
        {
            ...
            */
            ...
            double midRoofTopHoleDis =Convert.ToDouble(item.Deepth/2d - 400d - 360d - 75d -(int)((item.Deepth/2 - 400d - 360d - 75d - 50d) / 50d) * 50d) / 1000d;
            //KSA数量，KSA侧板长度(以全长计算)
            ...
            try
            {
                ...
                swModel.Parameter("D1@Distance49").SystemValue = (item.Deepth - 800d) / 2000d;
                ...
                if (item.ANSUL == "YES")
                {
                    ...
                    if ((meshSideLength - 20d / 1000d) > 57d / 1000d)
                    {
                        if (item.ANSide == "LEFT")
                        {
                            ...
                            swFeat.SetSuppression2(0, 2, null); //参数1：1解压，0压缩
                            //背面
                            swComp = swAssy.GetComponentByName(CommonFunc.AddSuffix(suffix, "FNHE0030-1"));
                            swComp.SetSuppression2(2); //2解压缩，0压缩
                            swPart = swComp.GetModelDoc2();
                            swPart.Parameter("D2@Sketch1").SystemValue = meshSideLength + 20d / 1000d;
                            swFeat = swComp.FeatureByName("ANSUL");
                            swFeat.SetSuppression2(1, 2, null); //参数1：1解压，0压缩
                            swComp = swAssy.GetComponentByName(CommonFunc.AddSuffix(suffix, "FNHE0029-1"));
                            swComp.SetSuppression2(2); //2解压缩，0压缩
                            swPart = swComp.GetModelDoc2();
                            swPart.Parameter("D2@Sketch1").SystemValue = meshSideLength - 20d / 1000d;
                            swFeat = swComp.FeatureByName("ANSUL");
                ...
                //----------排风腔内部零件----------
                //MESH油网下导轨
                swComp = swAssy.GetComponentByName(CommonFunc.AddSuffix(suffix, "FNHE0022-1"));
                swPart = swComp.GetModelDoc2();
                swPart.Parameter("D1@草图1").SystemValue = (item.Length - 8d) / 1000d;
                swComp = swAssy.GetComponentByName(CommonFunc.AddSuffix(suffix, "FNHE0023-1"));
                swPart = swComp.GetModelDoc2();
                swPart.Parameter("D1@草图1").SystemValue = (item.Length - 5d) / 1000d;
                swComp = swAssy.GetComponentByName(CommonFunc.AddSuffix(suffix, "FNHE0024-1"));
                swPart = swComp.GetModelDoc2();
                swPart.Parameter("D1@草图1").SystemValue = (item.Length - 3.5d) / 1000d;
                swComp = swAssy.GetComponentByName(CommonFunc.AddSuffix(suffix, "FNHE0025-1"));
                swPart = swComp.GetModelDoc2();
                swPart.Parameter("D1@草图1").SystemValue = (item.Length - 5d) / 1000d;
                ...
                swPart.Parameter("D2@草图1").SystemValue = ((item.Deepth - 800d - 720d) / 2d - 3d + 226d) / 1000d;
                swPart.Parameter("D1@草图6").SystemValue = ((item.Deepth - 800d - 720d) / 2d - 3d + 1d) / 1000d;
                swPart.Parameter("D3@草图25").SystemValue = midRoofTopHoleDis;
                if((item.Deepth - 800d - 720d) / 2d<150d)
                    swPart.Parameter("D2@草图26").SystemValue = ((item.Deepth - 800d - 720d) / 2d + 55d) / 3000d-30d/1000d;
                else
                    swPart.Parameter("D2@草图26").SystemValue = ((item.Deepth - 800d - 720d) / 2d + 55d) / 3000d;
                ...
                swEdit.FNHA0001(swComp, item.Length, frontPanelKaKouNo, frontPanelKaKouDis, midRoofSecondHoleDis, midRoofTopHoleDis, midRoofHoleNo, item.MARVEL, item.SidePanel, "UV", item.Bluetooth);
                

                //----------新风前面板----------
                swComp = swAssy.GetComponentByName(CommonFunc.AddSuffix(suffix, "FNHA0003-3"));
                swEdit.FNHA0003(swComp, item.Length, frontPanelKaKouNo, frontPanelKaKouDis, frontPanelHoleNo, frontPanelHoleDis);
                //----------新风底部CJ孔板----------
                swComp = swAssy.GetComponentByName(CommonFunc.AddSuffix(suffix, "FNHA0002-2"));
                swPart = swComp.GetModelDoc2();
                swPart.Parameter("D2@基体-法兰1").SystemValue = item.Length / 1000d;
                ...
                //------------I型新风腔主体----------
                swComp = swAssy.GetComponentByName(CommonFunc.AddSuffix(suffix, "FNHA0085-1"));
                swEdit.FNHA0001(swComp, item.Length, frontPanelKaKouNo, frontPanelKaKouDis, midRoofSecondHoleDis, midRoofTopHoleDis, midRoofHoleNo, "No", item.SidePanel, "KV", "No");

                //----------新风底部CJ孔板----------
                swComp = swAssy.GetComponentByName(CommonFunc.AddSuffix(suffix, "FNHA0086-1"));
                swPart = swComp.GetModelDoc2();
                swPart.Parameter("D2@基体-法兰1").SystemValue = item.Length / 1000d;
                swPart.Parameter("D1@CJHOLES").SystemValue = frontCjNo;
                swPart.Parameter("D10@草图8").SystemValue = frontCjFirstDis;
