    ...
    public class UVI555AutoDrawing : IAutoDrawing
    {
        ...
        public void AutoDrawing(SldWorks swApp, ModuleTree tree, string projectPath)
        {
            ...
            */
            ...
            double midRoofTopHoleDis = Convert.ToDouble(item.Deepth - 535d - 360d - 90d - (int)((item.Deepth - 535d - 360d - 90d - 100d) / 50d) * 50d) / 1000d;
            //KSA数量，KSA侧板长度(以全长计算)
            ...
            try
            {
                ...
                if (item.UVType == "LONG")
                {
                    swComp = swAssy.GetComponentByNameWithSuffix(suffix, "5201050414-1");
                    swComp.SetSuppression2(2); //2解压缩，0压缩
                    swComp = swAssy.GetComponentByNameWithSuffix(suffix, "5201060409-1");
                    swComp.SetSuppression2(2); //2解压缩，0压缩
                    swComp = swAssy.GetComponentByNameWithSuffix(suffix, "5201050413-1");
                    swComp.SetSuppression2(0); //2解压缩，0压缩
                ...
                if (item.ANSUL == "YES")
                {
                    ...
                    if ((meshSideLength - 20d / 1000d) > 57d / 1000d)
                    {
                        if (item.ANSide == "LEFT")
                        {
                            swComp = swAssy.GetComponentByNameWithSuffix(suffix, "FNHE0012-1");
                            swComp.SetSuppression2(2); //2解压缩，0压缩
                            swPart = swComp.GetModelDoc2();
