        ...
        public void AutoDrawing(SldWorks swApp, ModuleTree tree, string projectPath)
        {
            ...
            */
            ...
            try
            {
                //----------Top Level----------
                //烟罩深度
                swModel.Parameter("D1@Distance84").SystemValue = item.Deepth / 1000m;
                ...
                if (item.SidePanel == "BOTH")
                {
                    ...
                    swComp.SetSuppression2(2); //2解压缩，0压缩
                    swEdit.FNHS0055(swComp, item.Deepth, 450m, 300m, sidePanelSideCjNo, sidePanelDownCjNo);

                    swComp = swAssy.GetComponentByName(CommonFunc.AddSuffix(suffix, "FNHS0056-1"));
                    swComp.SetSuppression2(2); //2解压缩，0压缩
                    swEdit.FNHS0056(swComp, item.Deepth, 450m, 300m);

                    //RIGHT
                    swComp = swAssy.GetComponentByName(CommonFunc.AddSuffix(suffix, "FNHS0057-1"));
                    swComp.SetSuppression2(2); //2解压缩，0压缩
                    swEdit.FNHS0057(swComp, item.Deepth, 450m, 300m, sidePanelSideCjNo, sidePanelDownCjNo);

                    swComp = swAssy.GetComponentByName(CommonFunc.AddSuffix(suffix, "FNHS0058-1"));
                    swComp.SetSuppression2(2); //2解压缩，0压缩
                    swEdit.FNHS0058(swComp, item.Deepth, 450m, 300m);
                    ...
                    if (item.WaterCollection == "YES")
                    {
                        ...
                        swPart.Parameter("D2@Base-Flange1").SystemValue =
                            (item.Deepth - 278m) / 1000m; //水洗烟罩(item.Deepth - 368) / 1000m;
                        swPart.Parameter("D5@Sketch7").SystemValue =
                            15m / 1000m; //UV450400，15m，标准烟罩13.68m,水洗烟罩19.87m / 1000m
                ...
                else if (item.SidePanel == "LEFT")
                {
                    ...
                    if (item.WaterCollection == "YES")
                    {
                        ...
                        swComp.SetSuppression2(2); //2解压缩，0压缩
                        swPart = swComp.GetModelDoc2();
                        swPart.Parameter("D2@Base-Flange1").SystemValue = (item.Deepth - 278m) / 1000m;//水洗烟罩(item.Deepth - 368) / 1000m;
                ...
                swPart.Parameter("D3@Sketch11").SystemValue = 200m / 1000m - midRoofTopHoleDis;
                swFeat = swComp.FeatureByName("LPattern1");
                if (midRoofHoleNo == 1) swFeat.SetSuppression2(0, 2, configNames); //参数1：1解压，0压缩 
                else
                {
                ...
                if (item.MARVEL == "YES")
                {
                    if (item.IRNo > 0)
                    {
                        ...
                        swFeat.SetSuppression2(1, 2, configNames); //参数1：1解压，0压缩
                        swPart.Parameter("D2@Sketch19").SystemValue = item.IRDis1 / 1000m;
                    }
                    if (item.IRNo > 1)
                    {
                        ...
                        swFeat.SetSuppression2(1, 2, configNames); //参数1：1解压，0压缩
                        swPart.Parameter("D2@Sketch20").SystemValue = item.IRDis2 / 1000m;
                    }
                    if (item.IRNo > 2)
                    {
                        ...
                        swFeat.SetSuppression2(1, 2, configNames); //参数1：1解压，0压缩
                        swPart.Parameter("D2@Sketch21").SystemValue = item.IRDis3 / 1000m;
                    }
                }
                else
                {
