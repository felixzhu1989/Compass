        ...
        public void AutoDrawing(SldWorks swApp, ModuleTree tree, string projectPath)
        {
            ...
            */
            ...
            try
            {
                ...
                //KSA距离左边缘
                if (ksaSideLength < 12d) swModel.ChangeDim("D1@Distance71", 0.5d);
                else swModel.ChangeDim("D1@Distance71", ksaSideLength);

                //排风脖颈数量和距离
                if (item.ExNo == 1)
                {
                    swModel.ChangeDim("D1@Distance68", item.ExRightDis - item.ExLength / 2d);
                    swAssy.Suppress("LocalLPattern2");
                }
                else
                {
                    swModel.ChangeDim("D1@Distance68", item.ExRightDis - item.ExLength - item.ExDis / 2d);
                ...
                #region 排风腔
                //----------排风腔背部零件----------
                swComp = swAssy.GetComponentByNameWithSuffix(suffix, "FNHE0147-1");
                swEdit.FNHE0147(swComp, item.Length, item.WaterCollection, item.SidePanel, item.Outlet, item.BackToBack);
                ...
                swEdit.FNHE0149(swComp, item.Length, item.Inlet, item.UVType, item.ExRightDis);
                ...
                swEdit.FNHA0092(swComp, item.Length, frontPanelKaKouNo, frontPanelKaKouDis, midRoofSecondHoleDis, midRoofTopHoleDis, midRoofHoleNo, item.SuNo, item.SuDis, item.MARVEL, item.IRNo, item.IRDis1, item.IRDis2, item.IRDis3, item.Bluetooth, item.SidePanel);
                ...
                swEdit.FNHA0094(swComp, item.Length, frontPanelKaKouNo, frontPanelKaKouDis, frontPanelHoleNo, frontPanelHoleDis);

                //------------镀锌隔板----------
                swComp = swAssy.GetComponentByNameWithSuffix(suffix, "FNHA0099-1");
                swEdit.FNHA0099(swComp, item.Length, item.MARVEL, item.IRNo, item.IRDis1, item.IRDis2, item.IRDis3);

                //------------新风滑门导轨----------
                swComp = swAssy.GetComponentByNameWithSuffix(suffix, "FNHA0097-1");
                swEdit.FNHA0097(swComp, item.Length);

                #endregion F400型新风腔主体
