    ...
    public class HWUVF650AutoDrawing : IAutoDrawing
    {
        ...
        public void AutoDrawing(SldWorks swApp, ModuleTree tree, string projectPath)
        {
            ...
            try
            {
                #region Top Level
                //烟罩深度
                swModel.Parameter("D1@Distance85").SystemValue = item.Deepth / 1000d;
                ...
                if (item.ExNo == 1)
                {
                    swModel.Parameter("D1@Distance87").SystemValue = (item.ExRightDis - item.ExLength / 2) / 1000d;
                ...
                if (item.MARVEL == "YES")
                {
                    if (item.IRNo > 0)
                    {
                        ...
                        swFeat.SetSuppression2(1, 2, null); //参数1：1解压，0压缩
                        swPart.Parameter("D3@Sketch48").SystemValue = item.IRDis1 / 1000d;
                        swFeat = swComp.FeatureByName("MACABLE1");
                        swFeat.SetSuppression2(1, 2, null); //参数1：1解压，0压缩
                        if (item.SuNo == 1) swPart.Parameter("D1@Sketch50").SystemValue = (item.Length + 400) / 2000d;
                        else swPart.Parameter("D1@Sketch50").SystemValue = item.Length / 2000d;
                    }
                    if (item.IRNo > 1)
                    {
                        swFeat = swComp.FeatureByName("MA2");
                        swFeat.SetSuppression2(1, 2, null); //参数1：1解压，0压缩
                        swPart.Parameter("D3@Sketch51").SystemValue = item.IRDis2 / 1000d;
                        swFeat = swComp.FeatureByName("MACABLE2");
                        swFeat.SetSuppression2(1, 2, null); //参数1：1解压，0压缩
                        swPart.Parameter("D1@Sketch50").SystemValue = (item.Length / 2 - 25d) / 1000d;
                        swPart.Parameter("D1@Sketch52").SystemValue = 50d / 1000d;
                    }
                    if (item.IRNo > 2)
                    {
                        swFeat = swComp.FeatureByName("MA3");
                        swFeat.SetSuppression2(1, 2, null); //参数1：1解压，0压缩
                        swPart.Parameter("D3@Sketch53").SystemValue = item.IRDis3 / 1000d;
                        swFeat = swComp.FeatureByName("MACABLE3");
                        swFeat.SetSuppression2(1, 2, null); //参数1：1解压，0压缩
                        swPart.Parameter("D1@Sketch50").SystemValue = 220d / 1000d;
                        swPart.Parameter("D1@Sketch52").SystemValue = (item.Length - 440d) / 2000d;
                        swPart.Parameter("D1@Sketch54").SystemValue = (item.Length - 440d) / 2000d;
                    }
                }
                else
                {
