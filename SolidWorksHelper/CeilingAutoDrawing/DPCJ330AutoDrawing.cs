...
namespace SolidWorksHelper
{
    public class DPCJ330AutoDrawing : IAutoDrawing
    {
        ...
        public void AutoDrawing(SldWorks swApp, ModuleTree tree, string projectPath)
        {
            ...
            //查询参数
            DPCJ330 item = (DPCJ330)objDPCJ330Service.GetModelByModuleTreeId(tree.ModuleTreeId.ToString());
            ...
            */
            ...
            double leftSBDis = item.LeftDis;
            double rightSBDis = item.RightDis;

            try
            {
                //----------Top Level----------
                //----------CJ腔----------
                swComp = swAssy.GetComponentByName(CommonFunc.AddSuffix(suffix, "CJ330INDPCJ-1"));
                swSubAssy = swComp.GetModelDoc2(); //打开零件
                swSubModel = (ModelDoc2)swSubAssy;
                //----------脖颈----------
                if (item.SuType == "UP")
                {
                    swComp = swSubAssy.GetComponentByName(CommonFunc.AddSuffix(suffix, "FNCJ0010-1"));
                    swComp.SetSuppression2(2); //2解压缩，0压缩.
                    swComp = swSubAssy.GetComponentByName(CommonFunc.AddSuffix(suffix, "5201990413-1"));
                ...
                {
                    case "LEFT":
                        swComp = swSubAssy.GetComponentByName(CommonFunc.AddSuffix(suffix, "FNCJ0025-5"));
                        swComp.SetSuppression2(2); //2解压缩，0压缩.
                        swComp = swSubAssy.GetComponentByName(CommonFunc.AddSuffix(suffix, "FNCJ0027-3"));
                        swComp.SetSuppression2(2); //2解压缩，0压缩.
                        swComp = swSubAssy.GetComponentByName(CommonFunc.AddSuffix(suffix, "FNCJ0026-1"));
                        swComp.SetSuppression2(0); //2解压缩，0压缩.
                        swComp = swSubAssy.GetComponentByName(CommonFunc.AddSuffix(suffix, "FNCJ0027-4"));
                        swComp.SetSuppression2(0); //2解压缩，0压缩.
                        break;
                    case "RIGHT":
                        swComp = swSubAssy.GetComponentByName(CommonFunc.AddSuffix(suffix, "FNCJ0025-5"));
                ...
                //----------CJ腔主体----------
                swComp = swSubAssy.GetComponentByName(CommonFunc.AddSuffix(suffix, "FNCJ0023-2"));
                swPart = swComp.GetModelDoc2(); //打开零件
                swPart.Parameter("D2@Skizze1").SystemValue = item.Length / 1000d;
                swPart.Parameter("D1@LPattern7").SystemValue = cjNo + 1;
                swPart.Parameter("D3@Skizze18").SystemValue = firstCjDis / 1000d;
                ...
                swComp = swSubAssy.GetComponentByName(CommonFunc.AddSuffix(suffix, "FNCJ0022-1"));
                swPart = swComp.GetModelDoc2(); //打开零件
                swPart.Parameter("D4@Skizze1").SystemValue = item.Length / 1000d;
                swPart.Parameter("D4@Skizze3").SystemValue = item.SuDis / 1000d;
                ...
                //----------吊装扣板----------
                if (item.Length < 1000d)
                {
                    swSubModel.Parameter("D1@LocalLPattern2").SystemValue = 2;
                    swSubModel.Parameter("D3@LocalLPattern2").SystemValue = (item.Length - 400d) / 1000d;
                }
                else if (item.Length >= 1000d && item.Length < 2000d)
                {
                    swSubModel.Parameter("D1@LocalLPattern2").SystemValue = 3;
                    swSubModel.Parameter("D3@LocalLPattern2").SystemValue = (item.Length - 400d) / 2000d;
                }
                else
                {
                    swSubModel.Parameter("D1@LocalLPattern2").SystemValue = 4;
                    swSubModel.Parameter("D3@LocalLPattern2").SystemValue = (item.Length - 400d) / 3000d;
                }
                //----------侧板----------
                switch (item.SidePanel)
                {
                    case "LEFT":
                    case "DPBACKL":
                        swComp = swSubAssy.GetComponentByName(CommonFunc.AddSuffix(suffix, "FNCS0011-1"));
                        swComp.SetSuppression2(2); //2解压缩，0压缩.
                        swComp = swSubAssy.GetComponentByName(CommonFunc.AddSuffix(suffix, "FNCS0013-2"));
                        swComp.SetSuppression2(2); //2解压缩，0压缩.
                        swComp = swSubAssy.GetComponentByName(CommonFunc.AddSuffix(suffix, "FNCS0012-1"));
                        swComp.SetSuppression2(0); //2解压缩，0压缩.
                        swComp = swSubAssy.GetComponentByName(CommonFunc.AddSuffix(suffix, "FNCS0013-3"));
                        swComp.SetSuppression2(0); //2解压缩，0压缩.
                        break;
                    case "RIGHT":
                    case "DPBACKR":
                ...
                if (status)
                {
                    ...
                    if (item.BackCJSide == "LEFT" || item.BackCJSide == "BOTH")
                    {
                        swFeat = swComp.FeatureByName("BCJ-LEFT");
                        swFeat.SetSuppression2(1, 2, configNames); //参数1：1解压，0压缩
                        swPart.Parameter("D8@Sketch19").SystemValue = (item.LeftDis + 11d) / 1000d;
                        leftSBDis += 90d;
                    }
                    else
                    {
                    ...
                    if (item.BackCJSide == "RIGHT" || item.BackCJSide == "BOTH")
                    {
                        swFeat = swComp.FeatureByName("BCJ-RIGHT");
                        swFeat.SetSuppression2(1, 2, configNames); //参数1：1解压，0压缩
                        swPart.Parameter("D1@Sketch20").SystemValue = (item.RightDis + 11d) / 1000d;
                        rightSBDis += 90d;
                    }
                    else
                    {
                    ...
                    if (item.RightBeamType == "KCWDB800" || item.RightBeamType == "UCWDB800")
                    {
                        ...
                        if (item.GutterSide == "RIGHT" || item.GutterSide == "BOTH")
                        {
                            ...
                            swPart.Parameter("D7@Sketch55").SystemValue =
                                (item.Length - item.RightBeamDis + 1d) / 1000d;
                        }
                    }
                    else
                    {
