            ...
            */
            ...
            try
            {
                ...
                if (item.LightType == "HCL")
                {
                    ...
                    //先解压缩出来零件再改顶层装配关系
                    swModel.Parameter("D1@Distance41").SystemValue = item.LightPanelLeft / 1000d;
                    swModel.Parameter("D1@Distance42").SystemValue = item.LightPanelLeft / 1000d;
                    //镀锌片数量
                    ...
                    if (status)
                    {
                        ...
                        {
                            swFeat.SetSuppression2(1, 2, configNames); //参数1：1解压，0压缩
                            swPart.Parameter("D5@Sketch21").SystemValue = (item.LightPanelLeft - 150) / 1000d;
                        }
                        else
                        {
                            swFeat.SetSuppression2(0, 2, configNames); //参数1：1解压，0压缩
                        ...
                        {
                            swFeat.SetSuppression2(1, 2, configNames); //参数1：1解压，0压缩
                            swPart.Parameter("D5@Sketch22").SystemValue = (item.LightPanelRight - 150) / 1000d;
                        }
                        else
                        {
                            swFeat.SetSuppression2(0, 2, configNames); //参数1：1解压，0压缩
                    ...
                    {
                        swFeat.SetSuppression2(1, 2, configNames); //参数1：1解压，0压缩
                        swPart.Parameter("D5@Sketch25").SystemValue = (item.LightPanelLeft - 150) / 1000d;
                    }
                    else
                    {
                        swFeat.SetSuppression2(0, 2, configNames); //参数1：1解压，0压缩
                    ...
                    {
                        swFeat.SetSuppression2(1, 2, configNames); //参数1：1解压，0压缩
                        swPart.Parameter("D5@Sketch24").SystemValue = (item.LightPanelRight - 150) / 1000d;
                    }
                    else
                    {
                        swFeat.SetSuppression2(0, 2, configNames); //参数1：1解压，0压缩
                    ...
                    swPart.Parameter("D1@Skizze1").SystemValue =
                        (item.Length - item.LightPanelLeft - item.LightPanelRight) / 1000d;
                    ...
                    switch (item.LightPanelSide)
                    {
                        case "LEFT":
                            ...
                            if (status)
                            {
                                swComp = swAssy.GetComponentByName(compReName + "-1");
                                swPart = swComp.GetModelDoc2(); //打开零件
                                swPart.Parameter("D1@Sketch1").SystemValue = item.LightPanelLeft / 1000d;
                                swPart.Parameter("D6@Sketch42").SystemValue = (item.LightPanelLeft - 150) / 1000d;
                            }
                        ...
                        case "RIGHT":
                            ...
                            if (status)
                            {
                                swComp = swAssy.GetComponentByName(compReName + "-1");
                                swPart = swComp.GetModelDoc2(); //打开零件
                                swPart.Parameter("D1@Sketch1").SystemValue = item.LightPanelRight / 1000d;
                                swPart.Parameter("D6@Sketch42").SystemValue = (item.LightPanelRight - 150) / 1000d;
                            }
                            swComp = swAssy.GetComponentByName(CommonFunc.AddSuffix(suffix, "FNCE0092-1"));
                            swComp.SetSuppression2(0); //2解压缩，0压缩.
                            break;
                ...
                if (item.FCBlindNo > 0)
                {
                    ...
                    swModel.Parameter("D1@Distance35").SystemValue = item.FCSideLeft / 1000d;
                    swModel.Parameter("D1@Distance45").SystemValue = item.FCSideLeft / 1000d;
                }
                else
                {
                    swComp = swAssy.GetComponentByName(CommonFunc.AddSuffix(suffix, "FNCE0107[BP-500]{500}-1"));
                ...
                if (item.FCType == "KSA")
                {
                    ...
                    swModel.Parameter("D1@Distance37").SystemValue = (item.FCSideLeft + 500d * item.FCBlindNo) / 1000d;
                ...
                else
                {
                    ...
                    swModel.Parameter("D1@Distance36").SystemValue = (item.FCSideLeft + 500d * item.FCBlindNo) / 1000d;
                    swModel.Parameter("D1@Distance43").SystemValue = (item.FCSideLeft + 500d * item.FCBlindNo) / 1000d;
                    swComp = swAssy.GetComponentByName(CommonFunc.AddSuffix(suffix, "5202040401-1"));
                    swComp.SetSuppression2(0); //2解压缩，0压缩.
                    swFeat = swAssy.FeatureByName("LocalLPattern5");
                    swFeat.SetSuppression2(0, 2, configNames); //参数1：1解压，0压缩
                ...
                if (item.SSPType == "DOME")
                {
                    ...
                    if (item.Gutter == "YES") swModel.Parameter("D1@Distance34").SystemValue = item.GutterWidth / 1000d;
                    else swModel.Parameter("D1@Distance34").SystemValue = 0.5d / 1000d;
                }
                else
                {
                    swComp = swAssy.GetComponentByName(CommonFunc.AddSuffix(suffix, "FNCE0036-1"));
                    ...
                    swPart = swComp.GetModelDoc2();//打开零件
                    swPart.Parameter("D2@Sketch1").SystemValue = item.Length / 1000d;
                    if (item.Gutter == "YES") swModel.Parameter("D1@Distance32").SystemValue = item.GutterWidth / 1000d;
                    else swModel.Parameter("D1@Distance32").SystemValue = 0.5d / 1000d;
                    swComp = swAssy.GetComponentByName(CommonFunc.AddSuffix(suffix, "FNCE0035-1"));
                    swComp.SetSuppression2(0); //2解压缩，0压缩.
                }
                //----------日本项目需要压缩零件----------
