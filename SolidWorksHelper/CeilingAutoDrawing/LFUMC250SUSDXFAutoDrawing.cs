...
{
    public class LFUMC250SUSDXFAutoDrawing : IAutoDrawing
    {
        readonly LFUMC250SUSDXFService objLFUMC250SUSDXFService = new LFUMC250SUSDXFService();
        ...
        public void AutoDrawing(SldWorks swApp, ModuleTree tree, string projectPath)
        {
            ...
            //查询参数
            LFUMC250SUSDXF objLFUMC250SUSDXF = (LFUMC250SUSDXF)objLFUMC250SUSDXFService.GetModelByModuleTreeId(tree.ModuleTreeId.ToString());
            ...
            if (objProject.HoodType == "HoodParent")
            {
                ...
                foreach (var item in oldList)
                {
                    ...
                    {
                        ModuleTreeId = objLFUMC250SUSDXF.ModuleTreeId,
                        PartDescription = item.PartDescription,
                        Length = item.Length,
                        Width = item.Width,
                        Thickness = item.Thickness,
                        Quantity = item.Quantity * objLFUMC250SUSDXF.Quantity,//多个UCP
                ...
                {
                    throw new Exception("LFUMC250SUSDXF的HoodCutlist导入数据库失败" + ex.Message);
                }
            }
            #endregion
            ...
            if (objProject.HoodType == "Ceiling")
            {
                ...
                {
                    throw new Exception("LFUMC250SUSDXF的CeilingCutlist导入数据库失败" + ex.Message);
                }
            }
            #endregion
        }
