...
{
    public class NOCJ330AutoDrawing : IAutoDrawing
    {
        Component2 swComp; readonly NOCJ330Service objNOCJ330Service = new NOCJ330Service();
        public void AutoDrawing(SldWorks swApp, ModuleTree tree, string projectPath)
        {
            ...
            //查询参数
            NOCJ330 item = (NOCJ330)objNOCJ330Service.GetModelByModuleTreeId(tree.ModuleTreeId.ToString());
            ...
            */
            ...
            try
            {
                ...
                {
                    case "LEFT":
                        swComp = swAssy.GetComponentByName(CommonFunc.AddSuffix(suffix, "FNCJ0052-1"));
                        swComp.SetSuppression2(2); //2解压缩，0压缩.
                        swPart = swComp.GetModelDoc2();//打开零件
                        swPart.Parameter("D1@Skizze1").SystemValue = (item.Width - 2d) / 1000d;
                        swComp = swAssy.GetComponentByName(CommonFunc.AddSuffix(suffix, "FNCJ0054-1"));
                        swComp.SetSuppression2(0); //2解压缩，0压缩.
                        swComp = swAssy.GetComponentByName(CommonFunc.AddSuffix(suffix, "FNCJ0055-1"));
                        swComp.SetSuppression2(0); //2解压缩，0压缩.
                        swComp = swAssy.GetComponentByName(CommonFunc.AddSuffix(suffix, "FNCJ0053-1"));
                        swComp.SetSuppression2(0); //2解压缩，0压缩.
                        swComp = swAssy.GetComponentByName(CommonFunc.AddSuffix(suffix, "FNCJ0054-3"));
                        swComp.SetSuppression2(2); //2解压缩，0压缩.
                        swPart = swComp.GetModelDoc2();//打开零件
                        swPart.Parameter("D1@Skizze1").SystemValue = (item.Width - 2d) / 1000d;
                        swComp = swAssy.GetComponentByName(CommonFunc.AddSuffix(suffix, "FNCJ0056-1"));
                        swComp.SetSuppression2(0); //2解压缩，0压缩.
                        break;
                    case "RIGHT":
                        swComp = swAssy.GetComponentByName(CommonFunc.AddSuffix(suffix, "FNCJ0052-1"));
                    ...
                    case "NOCJBACKL":
                        ...
                        swPart = swComp.GetModelDoc2();//打开零件
                        swPart.Parameter("D1@Skizze1").SystemValue = (item.Width - 2d) / 1000d;
                        swPart.Parameter("D7@Edge-Flange1").SystemValue = item.Width / 1000d;
                        swComp = swAssy.GetComponentByName(CommonFunc.AddSuffix(suffix, "FNCJ0053-1"));
                        swComp.SetSuppression2(0); //2解压缩，0压缩.
                        swComp = swAssy.GetComponentByName(CommonFunc.AddSuffix(suffix, "FNCJ0054-3"));
                        swComp.SetSuppression2(2); //2解压缩，0压缩.
                    ...
                    case "NOCJBACKR":
                        ...
                        swPart = swComp.GetModelDoc2();//打开零件
                        swPart.Parameter("D1@Skizze1").SystemValue = (item.Width - 2d) / 1000d;
                        swComp = swAssy.GetComponentByName(CommonFunc.AddSuffix(suffix, "FNCJ0055-2"));
                        swComp.SetSuppression2(0); //2解压缩，0压缩.
                        swComp = swAssy.GetComponentByName(CommonFunc.AddSuffix(suffix, "FNCJ0053-1"));
                        swComp.SetSuppression2(0); //2解压缩，0压缩.
                        swComp = swAssy.GetComponentByName(CommonFunc.AddSuffix(suffix, "FNCJ0054-3"));
                ...
                //----------NOCJ腔主体----------
                //重命名装配体内部
                compReName = "FNCJ0051[NOCJ330-" + tree.Module + "]{" + (int)item.Length + "}(" + (int)item.Width + ")";
                status = swModelDocExt.SelectByID2(CommonFunc.AddSuffix(suffix, "FNCJ0051-1") + "@" + assyName, "COMPONENT", 0, 0, 0, false, 0, null, 0);
                ...
                if (status)
                {
                    ...
                    swPart.Parameter("D7@Kante-Lasche1").SystemValue = item.Width / 1000d;
                    //NOCJBACKL/R/B
                    ...
                    if (item.LeftBeamType == "KCWDB800" || item.LeftBeamType == "UCWDB800")
                    {
                        ...
                        swFeat.SetSuppression2(1, 2, null); //参数1：1解压，0压缩
                        swPart.Parameter("D31@Sketch32").SystemValue = (item.LeftBeamDis + 1d) / 1000d;
                        if (item.GutterSide == "LEFT" || item.GutterSide == "BOTH")
                        {
                            swFeat = swComp.FeatureByName("GUTTER-LEFT");
                            swFeat.SetSuppression2(1, 2, null); //参数1：1解压，0压缩 
                    ...
                    if (item.LeftBeamType == "KCWSB265")
                    {
                        ...
                        swFeat.SetSuppression2(1, 2, null); //参数1：1解压，0压缩
                        swPart.Parameter("D15@Sketch22").SystemValue = (leftSBDis + 1d) / 1000d;
                        if (item.LKSide == "LEFT" || item.LKSide == "BOTH")
                        {
                            swFeat = swComp.FeatureByName("LKS270-LEFT");
                            swFeat.SetSuppression2(1, 2, null); //参数1：1解压，0压缩 
                    ...
                    if (item.RightBeamType == "KCWDB800" || item.RightBeamType == "UCWDB800")
                    {
                        ...
                        swFeat.SetSuppression2(1, 2, null); //参数1：1解压，0压缩
                        swPart.Parameter("D31@Sketch45").SystemValue = (item.RightBeamDis + 1d) / 1000d;
                        if (item.GutterSide == "RIGHT" || item.GutterSide == "BOTH")
                        {
                            swFeat = swComp.FeatureByName("GUTTER-RIGHT");
                            swFeat.SetSuppression2(1, 2, null); //参数1：1解压，0压缩 
                    ...
                    if (item.RightBeamType == "KCWSB535" || item.RightBeamType == "UCWSB535")
                    {
                        ...
                        swFeat.SetSuppression2(1, 2, null); //参数1：1解压，0压缩
                        swPart.Parameter("D23@Sketch29").SystemValue = (rightSBDis + 1d) / 1000d;
                        if (item.GutterSide == "RIGHT" || item.GutterSide == "BOTH")
                        {
                            swFeat = swComp.FeatureByName("GUTTER-RIGHT");
                            swFeat.SetSuppression2(1, 2, null); //参数1：1解压，0压缩 
                    ...
                    if (item.RightBeamType == "KCWSB265")
                    {
                        ...
                        swFeat.SetSuppression2(1, 2, null); //参数1：1解压，0压缩
                        swPart.Parameter("D14@Sketch23").SystemValue = (rightSBDis + 1d) / 1000d;
                        if (item.LKSide == "RIGHT" || item.LKSide == "BOTH")
                        {
                            swFeat = swComp.FeatureByName("LKS270-RIGHT");
                            swFeat.SetSuppression2(1, 2, null); //参数1：1解压，0压缩 
