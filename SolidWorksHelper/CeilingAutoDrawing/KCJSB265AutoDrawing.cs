            ...
            */
            ...
            try
            {
                ...
                if (item.SSPType == "DOME")
                {
                    ...
                    if (item.Gutter == "YES") swModel.Parameter("D1@Distance12").SystemValue = item.GutterWidth / 1000m;
                    else swModel.Parameter("D1@Distance12").SystemValue = 0.5m / 1000m;
                }
                else
                {
                    swComp = swAssy.GetComponentByName(CommonFunc.AddSuffix(suffix, "FNCE0036-1"));
                    ...
                    swPart = swComp.GetModelDoc2();//打开零件
                    swPart.Parameter("D2@Sketch1").SystemValue = item.Length / 1000m;
                    if (item.Gutter == "YES") swModel.Parameter("D1@Distance2").SystemValue = item.GutterWidth / 1000m;
                    else swModel.Parameter("D1@Distance2").SystemValue = 0.5m / 1000m;
                    swComp = swAssy.GetComponentByName(CommonFunc.AddSuffix(suffix, "FNCE0035-1"));
                    swComp.SetSuppression2(0); //2解压缩，0压缩.
                }
