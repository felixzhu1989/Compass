            ...
            */
            ...
            try
            {
                ...
                if (item.FCBlindNo > 0)
                {
                    ...
                    swModel.Parameter("D1@Distance11").SystemValue = item.FCSideLeft / 1000d;
                }
                else
                {
                    swComp = swAssy.GetComponentByName(CommonFunc.AddSuffix(suffix, "FNCE0107[BP-500]{500}-1"));
                ...
                if (item.FCType == "KSA")
                {
                    ...
                    swModel.Parameter("D1@Distance9").SystemValue = (item.FCSideLeft + 500d * item.FCBlindNo) / 1000d;
                ...
                else
                {
                    ...
                    swModel.Parameter("D1@Distance10").SystemValue = (item.FCSideLeft + 500d * item.FCBlindNo) / 1000d;
                    swComp = swAssy.GetComponentByName(CommonFunc.AddSuffix(suffix, "5202040401-1"));
                    swComp.SetSuppression2(0); //2解压缩，0压缩.
                    swFeat = swAssy.FeatureByName("LocalLPattern4");
                    swFeat.SetSuppression2(0, 2, configNames); //参数1：1解压，0压缩
                ...
                switch (item.FCSide)
                {
                    ...
                    case "BOTH":
                        ...
                        else compReName = "FNCE0109[BP-" + tree.Module + "]{" + (int)(item.FCSideRight - 2d) + "}";
                        status = swModelDocExt.SelectByID2(CommonFunc.AddSuffix(suffix, "FNCE0109[BP-]{}-1") + "@" + assyName, "COMPONENT", 0, 0, 0, false, 0, null, 0);
                        if (status) swModelDocExt.RenameDocument(compReName);
                        swModel.ClearSelection2(true);
                        status = swModelDocExt.SelectByID2(compReName + "-1" + "@" + assyName, "COMPONENT", 0, 0, 0, false, 0, null, 0);
                ...
                else
                {
                    ...
                    if (status)
                    {
                        ...
                        swFeat = swComp.FeatureByName("Cut-Extrude4");
                        swFeat.SetSuppression2(1, 2, configNames); //参数1：1解压，0压缩
                        swPart.Parameter("D4@Sketch2").SystemValue = item.ExRightDis / 1000d;
                        swPart.Parameter("D1@Sketch2").SystemValue = item.ExLength / 1000d;
                        swPart.Parameter("D2@Sketch2").SystemValue = item.ExWidth / 1000d;
                        if (item.ANSUL == "YES")
                        {
                            //侧喷
                            if (item.ANSide == "LEFT")
                ...
                if (item.SSPType == "DOME")
                {
                    ...
                    if (item.Gutter == "YES") swModel.Parameter("D1@Distance12").SystemValue = item.GutterWidth / 1000d;
                    else swModel.Parameter("D1@Distance12").SystemValue = 0.5d / 1000d;
                }
                else
                {
                    swComp = swAssy.GetComponentByName(CommonFunc.AddSuffix(suffix, "FNCE0036-1"));
                    ...
                    swPart = swComp.GetModelDoc2();//打开零件
                    swPart.Parameter("D2@Sketch1").SystemValue = item.Length / 1000d;
                    if (item.Gutter == "YES") swModel.Parameter("D1@Distance2").SystemValue = item.GutterWidth / 1000d;
                    else swModel.Parameter("D1@Distance2").SystemValue = 0.5d / 1000d;
                    swComp = swAssy.GetComponentByName(CommonFunc.AddSuffix(suffix, "FNCE0035-1"));
                    swComp.SetSuppression2(0); //2解压缩，0压缩.
                }
