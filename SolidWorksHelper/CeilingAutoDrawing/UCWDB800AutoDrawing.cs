...
namespace SolidWorksHelper
{
    ...
    {
        Component2 swComp; readonly UCWDB800Service objUCWDB800Service = new UCWDB800Service();
            ...
            try
            {
                ...
                switch (item.SidePanel)
                {
                    ...
                    default:
                        ...
                        swComp = swAssy.GetComponentByName(CommonFunc.AddSuffix(suffix, "FNCE0017-7"));
                        swComp.SetSuppression2(0); //2解压缩，0压缩.
                        swComp = swAssy.GetComponentByName(CommonFunc.AddSuffix(suffix, "FNCE0017-10"));//3dm板
                        swComp.SetSuppression2(0); //2解压缩，0压缩.
                        swComp = swAssy.GetComponentByName(CommonFunc.AddSuffix(suffix, "FNCE0017-9"));
                        swComp.SetSuppression2(0); //2解压缩，0压缩.
                ...
                if (item.FCBlindNo > 0)
                {
                    ...
                    swModel.Parameter("D1@Distance71").SystemValue = item.FCSideLeft / 1000d;
                }
                else
                {
                    swComp = swAssy.GetComponentByName(CommonFunc.AddSuffix(suffix, "FNCE0037[BP-500]{500}-6"));
                ...
                swModel.Parameter("D1@Distance64").SystemValue = (item.FCSideLeft + 500d * item.FCBlindNo) / 1000d;
                ...
                if (item.Japan == "YES")
                {
                    ...
                    if (status)
                    {
                        ...
                        if (item.UVType == "LONG") swPart.Parameter("D7@Sketch15").SystemValue = 1200d / 1000d;
                        else swPart.Parameter("D7@Sketch15").SystemValue = 600d / 1000d;
                        if (item.UVType == "LONG") swPart.Parameter("D2@Sketch15").SystemValue = 1600d / 1000d;
                        else swPart.Parameter("D2@Sketch15").SystemValue = 893d / 1000d;
                        if (item.SensorNo == 1)
                        {
                            swFeat = swComp.FeatureByName("HALL SENSOR FRONT");
                            swFeat.SetSuppression2(1, 2, null); //参数1：1解压，0压缩
                            swPart.Parameter("D3@Sketch19").SystemValue = item.SensorDis1 / 1000d;
                            swFeat = swComp.FeatureByName("HALL SENSOR BACK");
                            swFeat.SetSuppression2(1, 2, null); //参数1：1解压，0压缩
                            swPart.Parameter("D3@Sketch20").SystemValue = item.SensorDis1 / 1000d;
                            swFeat = swComp.FeatureByName("SENSOR CABLE HOLE");
                            swFeat.SetSuppression2(1, 2, null); //参数1：1解压，0压缩
                            swPart.Parameter("D3@Sketch21").SystemValue = item.SensorDis1 / 1000d;
                        ...
                        else if (item.SensorNo > 1)
                        {
                            ...
                            swPart.Parameter("D3@LPattern1").SystemValue = item.SensorDis2 / 1000d;
                        }
                        else
                        {
                            swFeat = swComp.FeatureByName("HALL SENSOR FRONT");
                ...
                else
                {
                    ...
                    if (status)
                    {
                        ...
                        {
                            swFeat = swComp.FeatureByName("MA-NTC");
                            swFeat.SetSuppression2(0, 2, null); //参数1：1解压，0压缩s 
                        }
                        swFeat = swComp.FeatureByName("LF");
                        if (item.SidePanel == "LEFT" || item.SidePanel == "BOTH")
                            swFeat.SetSuppression2(1, 2, null); //参数1：1解压，0压缩
                ...
                if (item.SSPType == "DOME")
                {
                    ...
                    if (item.Gutter == "YES") swModel.Parameter("D1@Distance59").SystemValue = item.GutterWidth / 1000d;
                    else swModel.Parameter("D1@Distance59").SystemValue = 0.5d / 1000d;
                    swComp = swAssy.GetComponentByName(CommonFunc.AddSuffix(suffix, "FNCE0035-4"));
                    swComp.SetSuppression2(2); //2解压缩，0压缩.
                    if (item.Gutter == "YES") swModel.Parameter("D1@Distance58").SystemValue = item.GutterWidth / 1000d;
                    else swModel.Parameter("D1@Distance58").SystemValue = 0.5d / 1000d;
                }
                else
                {
                    swComp = swAssy.GetComponentByName(CommonFunc.AddSuffix(suffix, "FNCE0036-3"));
                    ...
                    swPart = swComp.GetModelDoc2();//打开零件
                    swPart.Parameter("D2@Sketch1").SystemValue = item.Length / 1000d;
                    if (item.Gutter == "YES") swModel.Parameter("D1@Distance54").SystemValue = item.GutterWidth / 1000d;
                    else swModel.Parameter("D1@Distance54").SystemValue = 0.5d / 1000d;
                    swComp = swAssy.GetComponentByName(CommonFunc.AddSuffix(suffix, "FNCE0036-4"));
                    swComp.SetSuppression2(2); //2解压缩，0压缩.
                    if (item.Gutter == "YES") swModel.Parameter("D1@Distance55").SystemValue = item.GutterWidth / 1000d;
                    else swModel.Parameter("D1@Distance55").SystemValue = 0.5d / 1000d;
                    swComp = swAssy.GetComponentByName(CommonFunc.AddSuffix(suffix, "FNCE0035-4"));
                    swComp.SetSuppression2(0); //2解压缩，0压缩.
                    swComp = swAssy.GetComponentByName(CommonFunc.AddSuffix(suffix, "FNCE0035-3"));
                    swComp.SetSuppression2(0); //2解压缩，0压缩.
