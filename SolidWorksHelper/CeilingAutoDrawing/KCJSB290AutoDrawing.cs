...
namespace SolidWorksHelper
{
    public class KCJSB290AutoDrawing : IAutoDrawing
    {
        ...
        public void AutoDrawing(SldWorks swApp, ModuleTree tree, string projectPath)
        {
            ...
            {
                //排风腔
                var swComp = ceilingPart.RenameComp(swModel, swAssy, suffix, "KCJSB290", tree.Module, "FNCE0127", 1, item.Length, 290);
                if (swComp != null)
                    ceilingPart.FNCE0127(swComp, item.Length, item.MARVEL, item.ANSUL, item.ANSide, item.ANDetector, item.Japan, item.ExRightDis, item.ExLength, item.ExWidth);

                //盲板
                ceilingPart.FCBlind(swModel, swAssy, suffix, item.FCBlindNo, item.FCSideLeft, "FNCE0107[BP-500]{500}-1", "LocalLPattern3", "D1@Distance9", "NO", "");

                //FC或者KSA
                ceilingPart.KSAorFC(swModel, swAssy, suffix, item.FCBlindNo, item.FCType, fcNo, item.FCSideLeft, "5202040401-1", "LocalLPattern2", "D1@Distance4", "KCJ FC FILTER-1", "LocalLPattern1", "D1@Distance3", "NO", "");
                ...
                //----------SSP灯板支撑条----------
                ceilingPart.SSPSupport(swModel, swAssy, suffix, item.Length, "NO", item.SSPType, item.Gutter, item.GutterWidth, "FNCE0035-1", "D1@Distance5", "", "FNCE0036-1", "D1@Distance7", "");
                
                //----------日本项目需要压缩零件----------
                if (item.Japan == "YES")
                {
                    //吊装垫片
                    swAssy.Suppress(suffix, "FNCE0070-1");
                ...
                {
                    //吊装垫片
                    swAssy.UnSuppress(suffix, "FNCE0070-1");
