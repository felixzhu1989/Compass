...
{
    public class LPZAutoDrawing : IAutoDrawing
    {
        Component2 swComp; readonly LPZService objLPZService = new LPZService();
        public void AutoDrawing(SldWorks swApp, ModuleTree tree, string projectPath)
        {
            ...
            //查询参数
            LPZ item = (LPZ)objLPZService.GetModelByModuleTreeId(tree.ModuleTreeId.ToString());
            ...
            */
            ...
            try
            {
                ...
                //----------W板----------
                //重命名装配体内部
                compReName = "FNCL0005[LPW-" + tree.Module + ".1]{" + (int)(item.Length-10d) + "}(" + (int)item.Width + ")";
                status = swModelDocExt.SelectByID2(CommonFunc.AddSuffix(suffix, "FNCL0005[LPW-]{}-1") + "@" + assyName, "COMPONENT", 0, 0, 0, false, 0, null, 0);
                ...
                {
                    //重命名装配体内部
                    compReName = "FNCL0007[LPZ-" + tree.Module + "]{"+(int)(item.Length - 10d) + "}";
                    status = swModelDocExt.SelectByID2(CommonFunc.AddSuffix(suffix, "FNCL0007[LPZ-]{}-1") + "@" + assyName, "COMPONENT", 0, 0, 0, false, 0, null, 0);
                    if (status) swModelDocExt.RenameDocument(compReName);
                    swModel.ClearSelection2(true);
                    status = swModelDocExt.SelectByID2(compReName + "-1" + "@" + assyName, "COMPONENT", 0, 0, 0, false, 0, null, 0);
                    swModel.ClearSelection2(true);
                ...
                {
                    swComp = swAssy.GetComponentByName(CommonFunc.AddSuffix(suffix, "FNCL0007[LPZ-]{}-1"));
