        ...
        public void AutoDrawing(SldWorks swApp, ModuleTree tree, string projectPath)
        {
            ...
            */
            ...
            try
            {
                ...
                swModel.Parameter("D1@Distance71").SystemValue = (item.FCSideLeft + 500m * item.FCBlindNo) / 1000m;
                #endregion
                ...
                if (item.LightType == "HCL")
                {
                    ...
                    switch (item.HCLSide)
                    {
                        case "LEFT":
                            ...
                            swModel.Parameter("D1@Distance77").SystemValue = item.HCLSideLeft / 1000m;
                            break;
                        case "RIGHT":
                ...
                if (item.SSPType == "DOME")
                {
                    ...
                    if (item.Gutter == "YES") swModel.Parameter("D1@Distance70").SystemValue = item.GutterWidth / 1000m;
                    else swModel.Parameter("D1@Distance70").SystemValue = 0.5m / 1000m;
                }
                else
                {
                    swComp = swAssy.GetComponentByName(CommonFunc.AddSuffix(suffix, "FNCE0036-1"));
                    ...
                    swPart = swComp.GetModelDoc2();//打开零件
                    swPart.Parameter("D2@Sketch1").SystemValue = item.Length / 1000m;
                    if (item.Gutter == "YES") swModel.Parameter("D1@Distance68").SystemValue = item.GutterWidth / 1000m;
                    else swModel.Parameter("D1@Distance68").SystemValue = 0.5m / 1000m;
                    swComp = swAssy.GetComponentByName(CommonFunc.AddSuffix(suffix, "FNCE0035-1"));
                    swComp.SetSuppression2(0); //2解压缩，0压缩.
                }
                #endregion
