            ...
            try
            {
                ...
                switch (item.SidePanel)
                {
                    ...
                    case "BOTH":
                        ...
                        break;
                    default:
                        swComp = swAssy.GetComponentByName(CommonFunc.AddSuffix(suffix, "FNCE0017-6"));//3mm板
                        swComp.SetSuppression2(0); //2解压缩，0压缩.
                        swComp = swAssy.GetComponentByName(CommonFunc.AddSuffix(suffix, "FNCE0017-7"));
                        swComp.SetSuppression2(0); //2解压缩，0压缩.
                ...
                if (item.FCBlindNo > 0)
                {
                    ...
                    swModel.Parameter("D1@Distance72").SystemValue = item.FCSideLeft / 1000m;
                }
                else
                {
                    swComp = swAssy.GetComponentByName(CommonFunc.AddSuffix(suffix, "FNCE0037[BP-500]{500}-7"));
                ...
                swModel.Parameter("D1@Distance73").SystemValue = (item.FCSideLeft + 500m * item.FCBlindNo) / 1000m;
                ...
                if (item.SSPType == "DOME")
                {
                    ...
                    if (item.Gutter == "YES") swModel.Parameter("D1@Distance65").SystemValue = item.GutterWidth / 1000m;
                    else swModel.Parameter("D1@Distance65").SystemValue = 0.5m / 1000m;
                    swComp = swAssy.GetComponentByName(CommonFunc.AddSuffix(suffix, "FNCE0035-2"));
                    swComp.SetSuppression2(2); //2解压缩，0压缩.
                    if (item.Gutter == "YES") swModel.Parameter("D1@Distance63").SystemValue = item.GutterWidth / 1000m;
                    else swModel.Parameter("D1@Distance63").SystemValue = 0.5m / 1000m;
                }
                else
                {
                    swComp = swAssy.GetComponentByName(CommonFunc.AddSuffix(suffix, "FNCE0036-3"));
                    ...
                    swPart = swComp.GetModelDoc2();//打开零件
                    swPart.Parameter("D2@Sketch1").SystemValue = item.Length / 1000m;
                    if (item.Gutter == "YES") swModel.Parameter("D1@Distance60").SystemValue = item.GutterWidth / 1000m;
                    else swModel.Parameter("D1@Distance60").SystemValue = 0.5m / 1000m;
                    swComp = swAssy.GetComponentByName(CommonFunc.AddSuffix(suffix, "FNCE0036-2"));
                    swComp.SetSuppression2(2); //2解压缩，0压缩.
                    if (item.Gutter == "YES") swModel.Parameter("D1@Distance61").SystemValue = item.GutterWidth / 1000m;
                    else swModel.Parameter("D1@Distance61").SystemValue = 0.5m / 1000m;
                    swComp = swAssy.GetComponentByName(CommonFunc.AddSuffix(suffix, "FNCE0035-2"));
                    swComp.SetSuppression2(0); //2解压缩，0压缩.
                    swComp = swAssy.GetComponentByName(CommonFunc.AddSuffix(suffix, "FNCE0035-3"));
                    swComp.SetSuppression2(0); //2解压缩，0压缩.
