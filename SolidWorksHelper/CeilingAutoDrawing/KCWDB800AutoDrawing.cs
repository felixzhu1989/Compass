...
namespace SolidWorksHelper
{
    public class KCWDB800AutoDrawing : IAutoDrawing
    {
        ...
        public void AutoDrawing(SldWorks swApp, ModuleTree tree, string projectPath)
        {
            ...
            //查询参数
            KCWDB800 item = (KCWDB800)objKCWDB800Service.GetModelByModuleTreeId(tree.ModuleTreeId.ToString());
            ...
            */
            ...
            try
            {
                ...
                {
                    case "LEFT":
                        swComp = swAssy.GetComponentByName(CommonFunc.AddSuffix(suffix, "FNCE0017-6"));
                        ...
                        swComp = swAssy.GetComponentByName(CommonFunc.AddSuffix(suffix, "FNCE0016-6"));
                        swComp.SetSuppression2(2); //2解压缩，0压缩.
                        swComp = swAssy.GetComponentByName(CommonFunc.AddSuffix(suffix, "FNCE0016-7"));
                        swComp.SetSuppression2(0); //2解压缩，0压缩.
                    ...
                    case "BOTH":
                        ...
                        break;
                    default:
                        swComp = swAssy.GetComponentByName(CommonFunc.AddSuffix(suffix, "FNCE0017-6"));//3dm板
                        swComp.SetSuppression2(0); //2解压缩，0压缩.
                        swComp = swAssy.GetComponentByName(CommonFunc.AddSuffix(suffix, "FNCE0017-7"));
                        swComp.SetSuppression2(0); //2解压缩，0压缩.
                        swComp = swAssy.GetComponentByName(CommonFunc.AddSuffix(suffix, "FNCE0017-8"));//3dm板
                        swComp.SetSuppression2(0); //2解压缩，0压缩.
                        swComp = swAssy.GetComponentByName(CommonFunc.AddSuffix(suffix, "FNCE0017-9"));
                        swComp.SetSuppression2(0); //2解压缩，0压缩.
                        ...
                        swComp = swAssy.GetComponentByName(CommonFunc.AddSuffix(suffix, "FNCE0016-6"));
                        swComp.SetSuppression2(0); //2解压缩，0压缩.
                        swComp = swAssy.GetComponentByName(CommonFunc.AddSuffix(suffix, "FNCE0016-7"));//边缘挡板
                        swComp.SetSuppression2(0); //2解压缩，0压缩.
                        swComp = swAssy.GetComponentByName(CommonFunc.AddSuffix(suffix, "FNCE0016-8"));
                        swComp.SetSuppression2(0); //2解压缩，0压缩.
                        swComp = swAssy.GetComponentByName(CommonFunc.AddSuffix(suffix, "FNCE0062-3"));//LEFT
                ...
                if (item.FCBlindNo > 0)
                {
                    swComp = swAssy.GetComponentByName(CommonFunc.AddSuffix(suffix, "FNCE0037[BP-500]{500}-7"));
                    swComp.SetSuppression2(2); //2解压缩，0压缩
                    swComp = swAssy.GetComponentByName(CommonFunc.AddSuffix(suffix, "FNCE0037[BP-500]{500}-9"));
                    swComp.SetSuppression2(2); //2解压缩，0压缩
                    swFeat = swAssy.FeatureByName("LocalLPattern5");
                    swFeat.SetSuppression2(1, 2, null); //参数1：1解压，0压缩
                    swModel.Parameter("D1@LocalLPattern5").SystemValue = item.FCBlindNo; //D1阵列数量,D3阵列距离
                    swModel.Parameter("D1@Distance72").SystemValue = item.FCSideLeft / 1000d;
                }
                else
                {
                    swComp = swAssy.GetComponentByName(CommonFunc.AddSuffix(suffix, "FNCE0037[BP-500]{500}-7"));
                ...
                swModel.Parameter("D1@Distance73").SystemValue = (item.FCSideLeft + 500d * item.FCBlindNo) / 1000d;
                ...
                switch (item.FCSide)
                {
                    case "LEFT":
                        ...
                        status = swModelDocExt.SelectByID2(CommonFunc.AddSuffix(suffix, "FNCE0058[BP-]{}-4") + "@" + assyName, "COMPONENT", 0, 0, 0, false, 0, null, 0);
                        if (status) swModelDocExt.RenameDocument(compReName);
                        swModel.ClearSelection2(true);
                        ...
                        if (status)
                        {
                            ...
                            swComp.SetSuppression2(2); //2解压缩，0压缩.
                            swPart = swComp.GetModelDoc2();//打开零件
                            swPart.Parameter("D2@Sketch1").SystemValue = (item.FCSideLeft - fcNo * 1d - 4d) / 1000d;
                        ...
                        break;
                    case "RIGHT":
                        swComp = swAssy.GetComponentByName(CommonFunc.AddSuffix(suffix, "FNCE0058[BP-]{}-4"));
                        swComp.SetSuppression2(0); //2解压缩，0压缩.
                        swComp = swAssy.GetComponentByName(CommonFunc.AddSuffix(suffix, "FNCE0058[BP-]{}-5"));
                        swComp.SetSuppression2(0); //2解压缩，0压缩.
                        //重命名装配体内部
                        ...
                        if (status)
                        {
                            ...
                            swComp.SetSuppression2(2); //2解压缩，0压缩.
                            swPart = swComp.GetModelDoc2();//打开零件
                            swPart.Parameter("D2@Sketch1").SystemValue = (item.FCSideRight - fcNo * 1d - 4d) / 1000d;
                            if (item.FCSideLeft < 100d)
                            {
                                swFeat = swComp.FeatureByName("Edge-Flange2");
                                swFeat.SetSuppression2(0, 2, null); //参数1：1解压，0压缩
                ...
                if (item.Japan == "YES")
                {
                    ...
                    if (status)
                    {
                        ...
                        swFeat.SetSuppression2(0, 2, null); //参数1：1解压，0压缩
                        swFeat = swComp.FeatureByName("SENSOR CABLE HOLES");
                        swFeat.SetSuppression2(0, 2, null); //参数1：1解压，0压缩
                        if (item.MARVEL == "YES")
                        {
                            swFeat = swComp.FeatureByName("MA-NTC");
                            swFeat.SetSuppression2(1, 2, null); //参数1：1解压，0压缩
                            swPart.Parameter("D2@Sketch25").SystemValue =
                    ...
                    switch (item.FCSide)
                    {
                        case "LEFT":
                            ...
                            swFeat.SetSuppression2(1, 2, null); //参数1：1解压，0压缩
                            swPart.Parameter("D5@Sketch13").SystemValue = (item.FCSideLeft - 50d) / 1000d;
                            swFeat = swComp.FeatureByName("FC LEFT B");
                            swFeat.SetSuppression2(1, 2, null); //参数1：1解压，0压缩
                            swPart.Parameter("D4@Sketch14").SystemValue = (item.FCSideLeft - 50d) / 1000d;
                        ...
                        case "RIGHT":
                            ...
                            swFeat = swComp.FeatureByName("FC RIGHT F");
                            swFeat.SetSuppression2(1, 2, null); //参数1：1解压，0压缩
                            swPart.Parameter("D6@Sketch15").SystemValue = (item.FCSideRight - 50d) / 1000d;
                            swFeat = swComp.FeatureByName("FC RIGHT B");
                            swFeat.SetSuppression2(1, 2, null); //参数1：1解压，0压缩
                            swPart.Parameter("D5@Sketch16").SystemValue = (item.FCSideRight - 50d) / 1000d;
                            break;
                        case "BOTH":
                            swFeat = swComp.FeatureByName("FC LEFT F");
                            swFeat.SetSuppression2(1, 2, null); //参数1：1解压，0压缩
                    ...
                    swFeat.SetSuppression2(1, 2, null); //参数1：1解压，0压缩
                    if (item.FCSide == "RIGHT") swPart.Parameter("D5@Sketch17").SystemValue = 25d / 1000d;
                    else swPart.Parameter("D5@Sketch17").SystemValue = (item.FCSideLeft + 25d) / 1000d;
                    swFeat = swComp.FeatureByName("FC FIRST B");
                    swFeat.SetSuppression2(1, 2, null); //参数1：1解压，0压缩
                    if (item.FCSide == "RIGHT") swPart.Parameter("D3@Sketch18").SystemValue = 25d / 1000d;
                    else swPart.Parameter("D3@Sketch18").SystemValue = (item.FCSideLeft + 25d) / 1000d;
                ...
                {
                    swComp = swAssy.GetComponentByName(CommonFunc.AddSuffix(suffix, "FNCE0036-2"));
                    swComp.SetSuppression2(0); //2解压缩，0压缩.
                    ...
                    if (item.Gutter == "YES") swModel.Parameter("D1@Distance65").SystemValue = item.GutterWidth / 1000d;
                    else swModel.Parameter("D1@Distance65").SystemValue = 0.5d / 1000d;
                    swComp = swAssy.GetComponentByName(CommonFunc.AddSuffix(suffix, "FNCE0035-2"));
                    swComp.SetSuppression2(2); //2解压缩，0压缩.
                    if (item.Gutter == "YES") swModel.Parameter("D1@Distance63").SystemValue = item.GutterWidth / 1000d;
                    else swModel.Parameter("D1@Distance63").SystemValue = 0.5d / 1000d;
                }
                else
                {
                    swComp = swAssy.GetComponentByName(CommonFunc.AddSuffix(suffix, "FNCE0036-3"));
                    ...
                    swPart = swComp.GetModelDoc2();//打开零件
                    swPart.Parameter("D2@Sketch1").SystemValue = item.Length / 1000d;
                    if (item.Gutter == "YES") swModel.Parameter("D1@Distance60").SystemValue = item.GutterWidth / 1000d;
                    else swModel.Parameter("D1@Distance60").SystemValue = 0.5d / 1000d;
                    swComp = swAssy.GetComponentByName(CommonFunc.AddSuffix(suffix, "FNCE0036-2"));
                    swComp.SetSuppression2(2); //2解压缩，0压缩.
                    if (item.Gutter == "YES") swModel.Parameter("D1@Distance61").SystemValue = item.GutterWidth / 1000d;
                    else swModel.Parameter("D1@Distance61").SystemValue = 0.5d / 1000d;
                    swComp = swAssy.GetComponentByName(CommonFunc.AddSuffix(suffix, "FNCE0035-2"));
                    swComp.SetSuppression2(0); //2解压缩，0压缩.
                    swComp = swAssy.GetComponentByName(CommonFunc.AddSuffix(suffix, "FNCE0035-3"));
                    swComp.SetSuppression2(0); //2解压缩，0压缩.
