...
{
    public class UCWUVR4SDXFAutoDrawing : IAutoDrawing
    {
        readonly UCWUVR4SDXFService objUCWUVR4SDXFService = new UCWUVR4SDXFService();
        ...
        public void AutoDrawing(SldWorks swApp, ModuleTree tree, string projectPath)
        {
            ...
            //查询参数
            UCWUVR4SDXF objUCWUVR4SDXF = (UCWUVR4SDXF)objUCWUVR4SDXFService.GetModelByModuleTreeId(tree.ModuleTreeId.ToString());
            ...
            if (objProject.HoodType == "HoodParent")
            {
                ...
                foreach (var item in oldList)
                {
                    ...
                    {
                        ModuleTreeId = objUCWUVR4SDXF.ModuleTreeId,
                        PartDescription = item.PartDescription,
                        Length = item.Length,
                        Width = item.Width,
                        Thickness = item.Thickness,
                        Quantity = item.Quantity * objUCWUVR4SDXF.Quantity,//多个
                ...
                {
                    throw new Exception("UCWUVR4SDXF的HoodCutlist导入数据库失败" + ex.Message);
                }
            }
            #endregion
            ...
            if (objProject.HoodType == "Ceiling")
            {
                ...
                foreach (var item in oldList)
                {
                    ceilingCutLists.Add(new CeilingCutList()
                    {
                        ...
                        Width = item.Width,
                        Thickness = item.Thickness,
                        Quantity = item.Quantity * objUCWUVR4SDXF.Quantity,//多个UCP
                        Materials = item.Materials,
                        PartNo = item.PartNo,
                        UserId = 1
                    });
                ...
                {
                    throw new Exception("UCWUVR4SDXF的CeilingCutlist导入数据库失败" + ex.Message);
                }
            }
            #endregion
        }
