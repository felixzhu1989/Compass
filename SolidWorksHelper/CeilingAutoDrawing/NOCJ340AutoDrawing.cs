...
{
    public class NOCJ340AutoDrawing : IAutoDrawing
    {
        Component2 swComp; readonly NOCJ340Service objNOCJ340Service = new NOCJ340Service();
        public void AutoDrawing(SldWorks swApp, ModuleTree tree, string projectPath)
        {
            ...
            //查询参数
            NOCJ340 item = (NOCJ340)objNOCJ340Service.GetModelByModuleTreeId(tree.ModuleTreeId.ToString());
            ...
            try
            {
                ...
                switch (item.SidePanel)
                {
                    ...
                    case "NOCJBACKL":
                        ...
                        swPart.Parameter("D8@Skizze1").SystemValue = (item.Width - 2d) / 1000d;
                        swComp = swAssy.GetComponentByName(CommonFunc.AddSuffix(suffix, "FNCS0020-10"));
                        swComp.SetSuppression2(0); //2解压缩，0压缩.
                        swComp = swAssy.GetComponentByName(CommonFunc.AddSuffix(suffix, "FNCS0018-2"));
                        swComp.SetSuppression2(0); //2解压缩，0压缩.
                        swComp = swAssy.GetComponentByName(CommonFunc.AddSuffix(suffix, "FNCS0020-11"));
                        swComp.SetSuppression2(2); //2解压缩，0压缩.
                        if (item.Width == 90d)
                        {
                ...
                //----------NOCJ腔主体----------
                //重命名装配体内部
                compReName = "FNCS0016[NOCJ340-" + tree.Module + "]{" + (int)item.Length + "}(" + (int)item.Width + ")";
                status = swModelDocExt.SelectByID2(CommonFunc.AddSuffix(suffix, "FNCS0016-1") + "@" + assyName, "COMPONENT", 0, 0, 0, false, 0, null, 0);
