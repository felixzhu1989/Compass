...
{
    public class NOCJ300AutoDrawing : IAutoDrawing
    {
        Component2 swComp; readonly NOCJ300Service objNOCJ300Service = new NOCJ300Service();
        public void AutoDrawing(SldWorks swApp, ModuleTree tree, string projectPath)
        {
            ...
            //查询参数
            NOCJ300 item = (NOCJ300)objNOCJ300Service.GetModelByModuleTreeId(tree.ModuleTreeId.ToString());
            ...
            */
            ...
            try
            {
                ...
                {
                    case "LEFT":
                        swComp = swAssy.GetComponentByName(CommonFunc.AddSuffix(suffix, "FNCJ0002-2"));
                        swComp.SetSuppression2(2); //2解压缩，0压缩.
                        swPart = swComp.GetModelDoc2();//打开零件
                        swPart.Parameter("D1@Skizze1").SystemValue = (item.Width - 2d) / 1000d;
                        swComp = swAssy.GetComponentByName(CommonFunc.AddSuffix(suffix, "FNCJ0007-2"));
                        swComp.SetSuppression2(0); //2解压缩，0压缩.
                        swComp = swAssy.GetComponentByName(CommonFunc.AddSuffix(suffix, "FNCJ0008-2"));
                        swComp.SetSuppression2(0); //2解压缩，0压缩.
                        swComp = swAssy.GetComponentByName(CommonFunc.AddSuffix(suffix, "FNCJ0006-1"));
                        swComp.SetSuppression2(0); //2解压缩，0压缩.
                        swComp = swAssy.GetComponentByName(CommonFunc.AddSuffix(suffix, "FNCJ0007-3"));
                        swComp.SetSuppression2(2); //2解压缩，0压缩.
                        swPart = swComp.GetModelDoc2();//打开零件
                        swPart.Parameter("D1@Skizze1").SystemValue = (item.Width - 2d) / 1000d;
                        swComp = swAssy.GetComponentByName(CommonFunc.AddSuffix(suffix, "FNCJ0009-2"));
                        swComp.SetSuppression2(0); //2解压缩，0压缩.
                        break;
                    case "RIGHT":
                        swComp = swAssy.GetComponentByName(CommonFunc.AddSuffix(suffix, "FNCJ0002-2"));
                ...
                //----------NOCJ腔主体----------
                //重命名装配体内部
                compReName = "FNCJ0001[NOCJ300-" + tree.Module + "]{" + (int)item.Length + "}("+(int)item.Width+")";
                status = swModelDocExt.SelectByID2(CommonFunc.AddSuffix(suffix, "FNCJ0001-1") + "@" + assyName, "COMPONENT", 0, 0, 0, false, 0, null, 0);
