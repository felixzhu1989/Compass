...
namespace SolidWorksHelper
{
    ...
    {
        Component2 swComp;
        readonly LLKSJService objLLKSJService = new LLKSJService();
        public void AutoDrawing(SldWorks swApp, ModuleTree tree, string projectPath)
        {
            ...
            //查询参数
            LLKSJ item = (LLKSJ)objLLKSJService.GetModelByModuleTreeId(tree.ModuleTreeId.ToString());
            ...
            */
            ...
            try
            {
                ...
                #region 边缘板
                swComp =  ceilingPart.RenameComp(swModel, swAssy, suffix, "LLKS", $"{tree.Module}.01", "FNCL0029", 3, item.LeftLength,
                    233);
                if (swComp != null)
                    ceilingPart.FNCL0026(swComp, item.LeftLength);

                swComp =  ceilingPart.RenameComp(swModel, swAssy, suffix, "LLKS", $"{tree.Module}.02", "FNCL0030", 2, item.RightLength,
                    233);
                if (swComp != null)
                ...
                if (item.LongGlassNo <2)
                {
                    ...
                    if (swModel.Extension.SelectByID2($"{CommonFunc.AddSuffix(suffix, "FNCL0031-7")}@{assyName}", "COMPONENT", 0, 0, 0, false, 0, null, 0))
                        swAssy.Suppress(suffix, "FNCL0031-7");
                }
                else
                {
                    ...
                    if (swModel.Extension.SelectByID2($"{CommonFunc.AddSuffix(suffix, "FNCL0031-7")}@{assyName}", "COMPONENT", 0, 0, 0, false, 0, null, 0))
                        swAssy.UnSuppress(suffix, "FNCL0031-7");
                }
                //阵列
                ...
                {
                    swAssy.Suppress(suffix, "2200600003-18");
                    if (swModel.Extension.SelectByID2($"{CommonFunc.AddSuffix(suffix, "FNCL0031-12")}@{assyName}", "COMPONENT", 0, 0, 0, false, 0, null, 0))
                        swAssy.Suppress(suffix, "FNCL0031-12");
                }
                else
                {
                    swAssy.UnSuppress(suffix, "2200600003-18");
                    if (swModel.Extension.SelectByID2($"{CommonFunc.AddSuffix(suffix, "FNCL0031-12")}@{assyName}", "COMPONENT", 0, 0, 0, false, 0, null, 0))
                        swAssy.UnSuppress(suffix, "FNCL0031-12");
                }
                if (item.ShortGlassNo == 0)
                {
                    swAssy.Suppress(suffix, "2200600032-1");
                    swAssy.Suppress(suffix, "2200600003-17");
                }
                else
                {
                    swAssy.UnSuppress(suffix, "2200600032-1");
                    swAssy.UnSuppress(suffix, "2200600003-17");
                }
                ...
                if (item.LongGlassNo > 1)
                {
                    swComp =  ceilingPart.RenameComp(swModel, swAssy, suffix, "LLKS", "MID", "FNCL0031", 7, item.MidLength,
                        233);
                    if (swComp != null)
                ...
                {
                    swComp =  ceilingPart.RenameComp(swModel, swAssy, suffix, "LLKS", "MID", "FNCL0031", 12, item.MidLength,
                        233);
                    if (swComp != null)
                        ceilingPart.FNCL0026(swComp, item.MidLength);
                }
