        ...
        public void AutoDrawing(SldWorks swApp, ModuleTree tree, string projectPath)
        {
            ...
            */
            ...
            try
            {
                ...
                //----------边缘板----------
                //重命名装配体内部
                compReName = "FNCL0029[LLKS-" + tree.Module + "]{" + (int)item.LeftLength + "}";
                status = swModelDocExt.SelectByID2(CommonFunc.AddSuffix(suffix, "FNCL0029[LLKS-]{}-3") + "@" + assyName, "COMPONENT", 0, 0, 0, false, 0, null, 0);
                ...
                if (status)
                {
                    ...
                    swPart.Parameter("D2@Skizze1").SystemValue = item.LeftLength / 1000m;
                }
                //重命名装配体内部
                compReName = "FNCL0030[LLKS-" + tree.Module + "]{" + (int)item.RightLength + "}";
                status = swModelDocExt.SelectByID2(CommonFunc.AddSuffix(suffix, "FNCL0030[LLKS-]{}-2") + "@" + assyName, "COMPONENT", 0, 0, 0, false, 0, null, 0);
                if (status) swModelDocExt.RenameDocument(compReName);
                swModel.ClearSelection2(true);
                ...
                if (status)
                {
                    ...
                    swPart = swComp.GetModelDoc2(); //打开零件
                    swPart.Parameter("D2@Skizze1").SystemValue = item.RightLength / 1000m;
                }
                //----------长玻璃----------
                ...
                if (item.LongGlassNo > 1)
                {
                    ...
                    swComp.SetSuppression2(2); //2解压缩，0压缩.
                    swComp = swAssy.GetComponentByName(CommonFunc.AddSuffix(suffix, "FNCL0031[LLKS-MID]{289}-7"));
                    swComp.SetSuppression2(2); //2解压缩，0压缩.
                    swComp = swAssy.GetComponentByName(CommonFunc.AddSuffix(suffix, "2200600003-16"));
                    swComp.SetSuppression2(2); //2解压缩，0压缩.
