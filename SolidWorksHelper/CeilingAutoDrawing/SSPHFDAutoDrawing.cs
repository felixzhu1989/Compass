...
namespace SolidWorksHelper
{
    ...
    {
        Component2 swComp; readonly SSPHFDService objSSPHFDService = new SSPHFDService();
        public void AutoDrawing(SldWorks swApp, ModuleTree tree, string projectPath)
        {
            ...
            */
            ...
            try
            {
                ...
                //----------边缘Z板----------
                swModel.Parameter("D1@Distance20").SystemValue = (item.MPanelNo * 2d - 1d) * 500d / 1000d;
                //左边
                if (item.LeftType == "Z")
                {
                    //重命名装配体内部
                    compReName = "FNCM0016[SSPDZ-" + tree.Module + ".L]{" + (int)(item.Length - 10d) + "}(" + (int)item.LeftLength + ")";
                ...
                else
                {
                    ...
                    //重命名装配体内部
                    compReName = "FNCM0014[SSPDW-" + tree.Module + ".L]{" + (int)(item.Length - 10d) + "}(" + (int)item.LeftLength + ")";
                    status = swModelDocExt.SelectByID2(CommonFunc.AddSuffix(suffix, "FNCM0014-L[SSPDW-]{}()-1") + "@" + assyName, "COMPONENT", 0, 0, 0, false, 0, null, 0);
                    if (status) swModelDocExt.RenameDocument(compReName);
                    swModel.ClearSelection2(true);
                    status = swModelDocExt.SelectByID2(compReName + "-1" + "@" + assyName, "COMPONENT", 0, 0, 0, false, 0, null, 0);
                ...
                {
                    //重命名装配体内部
                    compReName = "FNCM0016[SSPDZ-" + tree.Module + ".R]{" + (int)(item.Length - 10d) + "}(" + (int)item.RightLength + ")";
                ...
                else
                {
                    ...
                    swComp.SetSuppression2(0); //2解压缩，0压缩.
                    //重命名装配体内部
                    compReName = "FNCM0014[SSPDW-" + tree.Module + ".R]{" + (int)(item.Length - 10d) + "}(" + (int)item.RightLength + ")";
                    status = swModelDocExt.SelectByID2(CommonFunc.AddSuffix(suffix, "FNCM0014-R[SSPDW-]{}()-1") + "@" + assyName, "COMPONENT", 0, 0, 0, false, 0, null, 0);
                    if (status) swModelDocExt.RenameDocument(compReName);
                    swModel.ClearSelection2(true);
                    status = swModelDocExt.SelectByID2(compReName + "-1" + "@" + assyName, "COMPONENT", 0, 0, 0, false, 0, null, 0);
                ...
                //----------标准M板----------
                //重命名装配体内部
                compReName = "FNCM0015[SSPDM-" + tree.Module + "]{" + (int)(item.Length - 10d) + "}(500)";
                ...
                {
                    //重命名装配体内部
                    compReName = "FNCM0014[SSPDW-" + tree.Module + "]{" + (int)(item.Length - 10d) + "}(500)";
