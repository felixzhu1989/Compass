...
namespace SolidWorksHelper
{
    public class UCWUVR4LDXFAutoDrawing : IAutoDrawing
    {
        ...
        public void AutoDrawing(SldWorks swApp, ModuleTree tree, string projectPath)
        {
            ...
            //查询参数
            UCWUVR4LDXF objUCWUVR4LDXF = (UCWUVR4LDXF)objUCWUVR4LDXFService.GetModelByModuleTreeId(tree.ModuleTreeId.ToString());
            ...
            if (objProject.HoodType == "HoodParent")
            {
                ...
                foreach (var item in oldList)
                {
                    ...
                    {
                        ModuleTreeId = objUCWUVR4LDXF.ModuleTreeId,
                        PartDescription = item.PartDescription,
                        Length = item.Length,
                        Width = item.Width,
                        Thickness = item.Thickness,
                        Quantity = item.Quantity * objUCWUVR4LDXF.Quantity,//多个UCP
                ...
                {
                    throw new Exception("UCWUVR4LDXF的HoodCutlist导入数据库失败" + ex.Message);
                }
            }
            #endregion
            ...
            if (objProject.HoodType == "Ceiling")
            {
                ...
                {
                    throw new Exception("UCWUVR4LDXF的CeilingCutlist导入数据库失败" + ex.Message);
                }
            }
            #endregion
        }
