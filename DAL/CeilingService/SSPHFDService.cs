...
namespace DAL
{
    public class SSPHFDService : IModelService
    {
        /// <summary>
        /// 根据项目Id查询SSPHFD集合
        /// </summary>
        ...
        {
            string sql = "select SSPHFDId,SSPHFD.ModuleTreeId,Length,LeftType,RightType,LeftLength,RightLength,MPanelNo,LightType from SSPHFD";
            sql += " inner join ModuleTree on SSPHFD.ModuleTreeId=ModuleTree.ModuleTreeId";
        ...
        {
            return GetModelByWhereSql(string.Format(" where SSPHFDId={0}", id));
        }
        /// <summary>
        /// 根据模型树ID返回SSPHFD
        /// </summary>
        ...
        {
            string sql =
                "select SSPHFDId,ModuleTreeId,Length,LeftType,RightType,LeftLength,RightLength,MPanelNo,LightType from SSPHFD";
            sql += whereSql;
            SqlDataReader objReader = SQLHelper.GetReader(sql);
            SSPHFD objModel = null;
            if (objReader.Read())
            {
                objModel = new SSPHFD()
                {
                    SSPHFDId = Convert.ToInt32(objReader["SSPHFDId"]),
        ...
        {
            SSPHFD objModel = (SSPHFD)models;
            //编写带参数的SQL语句
            StringBuilder sqlBuilder = new StringBuilder();
            sqlBuilder.Append("Update SSPHFD set Length=@Length,LeftType=@LeftType,RightType=@RightType,LeftLength=@LeftLength,RightLength=@RightLength,");
            sqlBuilder.Append("MPanelNo=@MPanelNo,LightType=@LightType where SSPHFDId=@SSPHFDId");
            //定义参数数组
            SqlParameter[] param = new SqlParameter[]
            {
                ...
                new SqlParameter("@SSPHFDId",objModel.SSPHFDId)
            };
            try
            {
