...
namespace DAL
{
    public class NOCJSPECService:IModelService
    {
        ...
        {
            return GetModelByWhereSql(string.Format(" where NOCJSPECId={0}", id));
        }
        ...
        public IModel GetModelByWhereSql(string whereSql)
        {
            ...
            if (objReader.Read())
            {
                objModel = new NOCJSPEC()
                {
                    ...
                    TopBend = objReader["TopBend"].ToString().Length == 0 ? 0 : Convert.ToDecimal(objReader["TopBend"]),
                    BackBend = objReader["BackBend"].ToString().Length == 0 ? 0 : Convert.ToDecimal(objReader["BackBend"]),
