...
namespace DAL
{
    public class UVI555Service : IModelService
    {
        ...
        {
            string sql = "select UVI555Id,UVI555.ModuleTreeId,Item,Module,Length,Deepth,ExRightDis,ExNo,EXDis,ExLength,ExWidth,ExHeight,SidePanel,Outlet,LEDlogo,Bluetooth,BackToBack,WaterCollection,LEDSpotNo,LEDSpotDis,LightType,UVType,ANSUL,ANSide,ANDetector,ANYDis,ANDropNo,ANDropDis1,ANDropDis2,ANDropDis3,ANDropDis4,ANDropDis5,MARVEL,IRNo,IRDis1,IRDis2,IRDis3 from UVI555";
            sql += " inner join ModuleTree on UVI555.ModuleTreeId=ModuleTree.ModuleTreeId";
        ...
        {
            return GetModelByWhereSql(string.Format(" where UVI555Id={0}", id));
        }
        /// <summary>
        /// 根据模型树ID返回UVI555
        /// </summary>
        ...
        {
            string sql =
                "select UVI555Id,ModuleTreeId,Length,Deepth,ExRightDis,ExNo,EXDis,ExLength,ExWidth,ExHeight,SidePanel," +
                ...
                "MARVEL,IRNo,IRDis1,IRDis2,IRDis3 from UVI555";
            sql += whereSql;
            SqlDataReader objReader = SQLHelper.GetReader(sql);
            UVI555 objModel = null;
            if (objReader.Read())
            {
                objModel=new UVI555()
                {
                    UVI555Id=Convert.ToInt32(objReader["UVI555Id"]),
        ...
        {
            UVI555 objModel = (UVI555) model;
            //编写带参数的SQL语句
            StringBuilder sqlBuilder=new StringBuilder();
            sqlBuilder.Append("Update UVI555 set Length=@Length,Deepth=@Deepth,ExRightDis=@ExRightDis,ExNo=@ExNo,ExDis=@ExDis,ExLength=@ExLength,ExWidth=@ExWidth,ExHeight=@ExHeight,SidePanel=@SidePanel,");
            ...
            sqlBuilder.Append("MARVEL=@MARVEL,IRNo=@IRNo,IRDis1=@IRDis1,IRDis2=@IRDis2,IRDis3=@IRDis3 where UVI555Id=@UVI555Id");
            //定义参数数组
            SqlParameter[] param=new SqlParameter[]
            {
                ...
                new SqlParameter("@UVI555Id",objModel.UVI555Id)
            };
            try
            {
