...
namespace DAL
{
    public class KVIMR555Service : IModelService
    {
        ...
        {
            string sql = "select KVIMR555Id,KVIMR555.ModuleTreeId,Item,Module,Length,Deepth,ExRightDis,ExNo,EXDis,ExLength,ExWidth,ExHeight,SidePanel,Outlet,LEDlogo,WaterCollection,LEDSpotNo,LEDSpotDis,LightType,ANSUL,ANSide,ANDetector,ANYDis,ANDropNo,ANDropDis1,ANDropDis2,ANDropDis3,ANDropDis4,ANDropDis5,MARVEL,IRNo,IRDis1,IRDis2,IRDis3 from KVIMR555";
            sql += " inner join ModuleTree on KVIMR555.ModuleTreeId=ModuleTree.ModuleTreeId";
        ...
        /// <summary>
        /// 根据KVIMR555ID返回KVIMR555
        /// </summary>
        /// <param name="id"></param>
        /// <returns></returns>
        public IModel GetModelById(string id)
        {
            return GetModelByWhereSql(string.Format(" where KVIMR555Id={0}", id));
        }
        /// <summary>
        /// 根据模型树ID返回KVIMR555
        /// </summary>
        ...
        {
            string sql =
                "select KVIMR555Id,ModuleTreeId,Length,Deepth,ExRightDis,ExNo,EXDis,ExLength,ExWidth,ExHeight,SidePanel," +
                "Outlet,LEDlogo,WaterCollection,LEDSpotNo,LEDSpotDis,LightType," +
                "ANSUL,ANSide,ANDetector,ANYDis,ANDropNo,ANDropDis1,ANDropDis2,ANDropDis3,ANDropDis4,ANDropDis5," +
                "MARVEL,IRNo,IRDis1,IRDis2,IRDis3 from KVIMR555";
            sql += whereSql;
            SqlDataReader objReader = SQLHelper.GetReader(sql);
            KVIMR555 objModel = null;
            if (objReader.Read())
            {
                objModel = new KVIMR555()
                {
                    KVIMR555Id = Convert.ToInt32(objReader["KVIMR555Id"]),
        ...
        {
            KVIMR555 objModel = (KVIMR555)model;
            //编写带参数的SQL语句
            StringBuilder sqlBuilder = new StringBuilder();
            sqlBuilder.Append("Update KVIMR555 set Length=@Length,Deepth=@Deepth,ExRightDis=@ExRightDis,ExNo=@ExNo,ExDis=@ExDis,ExLength=@ExLength,ExWidth=@ExWidth,ExHeight=@ExHeight,SidePanel=@SidePanel,");
            sqlBuilder.Append("Outlet=@Outlet,LEDlogo=@LEDlogo,WaterCollection=@WaterCollection,LEDSpotNo=@LEDSpotNo,LEDSpotDis=@LEDSpotDis,LightType=@LightType,");
            ...
            sqlBuilder.Append("MARVEL=@MARVEL,IRNo=@IRNo,IRDis1=@IRDis1,IRDis2=@IRDis2,IRDis3=@IRDis3 where KVIMR555Id=@KVIMR555Id");
            //定义参数数组
            SqlParameter[] param = new SqlParameter[]
            {
                ...
                new SqlParameter("@KVIMR555Id",objModel.KVIMR555Id)
            };
            try
            {
